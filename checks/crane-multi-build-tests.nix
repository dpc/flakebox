...
pkgs.runCommand "crane-multi-build-tests"
  ...
  ''
    ...
    if ! nix-instantiate --eval --strict --expr \
      "import $fixture { craneMultiBuildPath = $crane_multi_build; scenario = \"scope\"; }" \
      >/dev/null 2>scope-trace; then
      cat scope-trace >&2
      exit 1
    fi
    ...
    check_diagnostic() {
      ...
      if nix-instantiate --eval --strict --expr \
        "import $fixture { craneMultiBuildPath = $crane_multi_build; scenario = \"$scenario\"; }" \
        >"$scenario-out" 2>"$scenario-err"; then
