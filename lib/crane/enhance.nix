  ...
  self: prev: {
    ...
    mkCargoDerivation =
      ...
      let
        ...
        argsWithBuildTarget =
          mergedArgs
          // (lib.optionalAttrs (mergedArgs ? CARGO_BUILD_TARGET) {
            pname = "${mergedArgs.pname}-${mergedArgs.CARGO_BUILD_TARGET}";
          });
        argsWithBuildProfile =
          argsWithBuildTarget
          // (lib.optionalAttrs (mergedArgs.CARGO_PROFILE or "release" != "release") {
    ...
    buildPackage =
      ...
      prev.buildPackage (
        let
          mergedArgs = mergeArgs self.args args;
        in
        (
          mergedArgs
          // {
            ...
            cargoArtifacts =
              mergedArgs.cargoArtifacts or (self.buildDepsOnly (
                mergedArgs
                // {
      ...
      let
        ...
        pname =
          if builtins.hasAttr "pname" origArgs then
            "${origArgs.pname}-workspace"
          else if builtins.hasAttr "pname" self.args then
            "${self.args.pname}-workspace"
          else
            null;
      in
      self.buildDepsOnly (
    ...
    overrideArgsDepsOnly =
      args: self.overrideScope (self: prev: { argsDepsOnly = mergeArgs prev.argsDepsOnly args; });
    overrideArgsDepsOnly' =
      f: self.overrideScope (self: prev: { argsDepsOnly = prev.argsDepsOnly // f prev.argsDepsOnly; });
    overrideArgsDepsOnly'' =
      f:
      self.overrideScope (self: prev: { argsDepsOnly = prev.argsDepsOnly // f self prev.argsDepsOnly; });
    ...
    mapWithProfiles =
      ...
      builtins.listToAttrs (
        builtins.map (cargoProfile: {
          name = cargoProfile;
          value = f (self.overrideProfile cargoProfile);
        }) profiles
      );
    mapWithToolchains =
      f: toolchains:
    ...
    mapWithToolchains' =
      ...
      builtins.mapAttrs (
        ...
        f toolchainName (
          (self.overrideToolchainName toolchainName).overrideArgs (
            { inherit toolchainName; } // toolchain.args
          )
        )
      ) toolchains;
  }
